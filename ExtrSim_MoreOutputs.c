...
{ // gets energy of a potential cohesin bond
    double thisE = 0;
    double *coordsA;
    double *coordsB;
...
int *getNewBond(int *NewLoc, double JumpProb, int *OldBond, int *CohLocs, double *AllCoords, int *yesstop, int NumBeads, double K, double R0, int yesring)
{ // get the new indices of the cohesin bond
    double ProbFactor;
    ...
    {                     // otherwise, try to move the bond
        ...
        { // for each domain
            ...
            {                       // if try to move
                ...
                for (int vv = 0; vv < NumBeads; vv++)
                { // go through CohLocs and see if trial bead is already occupied
                    if (CohLocs[vv] == Move1temp[ww] || CohLocs[vv + NumBeads] == Move1temp[ww])
                    {
        ...
        // if both elements of Move1 are same bead, reject and keep old bond.
        if (Move1[0] == Move1[1])
        {
            NewLoc[0] = OldBond[0];
            NewLoc[1] = OldBond[1];
...
double *getCohCOM(double *CohCOM, double *AllCoords, int CL1, int CL2)
{ // get COM of coh beads
    double *cohcoord1;
    double *cohcoord2;
    cohcoord1 = (double *)malloc(3 * sizeof(double));
    cohcoord2 = (double *)malloc(3 * sizeof(double));
    cohcoord1 = getcoords(cohcoord1, CL1, AllCoords);
    cohcoord2 = getcoords(cohcoord2, CL2, AllCoords);
    for (int rr = 0; rr < 3; rr++)
    {
        CohCOM[rr] = (cohcoord1[rr] + cohcoord2[rr]) / 2;
    }
    free(cohcoord1);
    free(cohcoord2);
    return CohCOM;
...
int main(int narg, char **argv)
{
    ...
    // Assign MPI ranks if more than one rank.
    int statusrank = 0;
    int contactrank = 0;
    int R2rank = 0;
    if (nprocs > 2)
    ...
    if (myrank == statusrank)
    {
        ...
        if (statusFile == NULL)
        {
            fprintf(stdout, "statusfile failed line 440\n");
            return 1;
        }
    ...
    { // If stop signs provided
        ...
        if (allstopFile == NULL)
        {
            fprintf(stdout, "allstop failed line 472\n");
            return 1;
        }
        ...
        if (myrank == statusrank)
        {
            ...
            if (statusFile == NULL)
            {
                fprintf(stdout, "statusfile failed line 489\n");
                return 1;
            }
    ...
    if (myrank == statusrank)
    {
        ...
        if (statusFile == NULL)
        {
            fprintf(stdout, "statusfile failed line 540\n");
            return 1;
        }
    ...
    FILE *BindFile;
    char cohlocname[500];
    char cohloceachname[500];
    char cohlifename[500];
    char finallifename[500];
    char numcohname[500];
    char cohflucname[500];
    char accfilename[500];
    char bindfilename[500];
    if (myrank == 0)
    {
        sprintf(cohlocname, "%sCohLocs_%s.txt", WorkPath, outputfilenames);
        sprintf(cohlifename, "%sCohLife_%s.txt", WorkPath, outputfilenames);
    ...
    if (myrank == statusrank)
    {
        ...
        if (statusFile == NULL)
        {
            fprintf(stdout, "statusfile failed line 598\n");
            return 1;
        }
        ...
        if (ffcc != 0)
        {
            fprintf(stdout, "fclose failed 605");
            return (1);
        }
    }
    ...
    if (lmp == NULL)
    {
        if (myrank == 0)
        {
            ...
            if (statusFile == NULL)
            {
                fprintf(stdout, "statusfile failed line 625\n");
                return 1;
            }
            ...
            if (ffcc != 0)
            {
                fprintf(stdout, "fclose failed 632");
                return (1);
            }
        }
        lammps_close(lmp);
    ...
    else
    {
        if (myrank == 0)
        {
            ...
            if (statusFile == NULL)
            {
                fprintf(stdout, "statusfile failed line 646\n");
                return 1;
            }
            ...
            if (ffcc != 0)
            {
                fprintf(stdout, "fclose failed 653");
                return (1);
            }
        }
    }
    ...
    if (myrank == statusrank)
    {
        ...
        if (statusFile == NULL)
        {
            fprintf(stdout, "statusfile failed line 686\n");
            return 1;
        }
        ...
        if (ffcc != 0)
        {
            fprintf(stdout, "fclose failed 693");
            return (1);
        }
    }
    ...
    sprintf(vxdiff, "variable xdiff equal c_2[%d]-c_2[1]", NumBeads);
    ...
    if (myrank == 0)
    {
        ...
        if (statusFile == NULL)
        {
            fprintf(stdout, "statusfile failed line 730\n");
            return 1;
        }
        ...
        if (ffcc != 0)
        {
            fprintf(stdout, "fclose failed 737");
            return (1);
        }
    }
    ...
    lammps_command(lmp, "log none"); // comment out this line to continue LAMMPS logfile outputs, useful for debugging
    ...
    // allocate memory for coh COM
    double *CohCOM;
    CohCOM = (double *)malloc(3 * sizeof(double));

    // allocate memory for contact matrices. one for overall, one for intermittent.
    int *contactmat;
    ...
    if (myrank == statusrank)
    {
        ...
        if (statusFile == NULL)
        {
            fprintf(stdout, "statusfile failed line 794\n");
            return 1;
        }
        ...
        if (ffcc != 0)
        {
            fprintf(stdout, "fclose failed 801");
            return (1);
        }
    }
    ...
    char dumpbindcommand[200]; // for each binding event
    ...
    {                                 // for each MC step
        ...
        int *newBondsAll, *delBondsAll;
        int *newBonds; // array for new bonds. make larger just in case
        newBonds = (int *)malloc(sizeof(int) * 4 * maxCoh);
        for (int ii = 0; ii < 4 * maxCoh; ii++)
        {
            newBonds[ii] = 0;
        }
        int *delBonds; // array for deleting bonds
        ...
        int *indwhereNew; // indices for new bonds
        int *indwhereDel; // indices for delete bonds
        ...
        // only do MC calculation on root
        if (myrank == 0)
        {
            ...
            // end = add coh
            int delIndex[maxCoh]; // if deleted that coh, 0 otherwise.
                    ...
                    { // try to unbind first
                        ...
                        { // if something is there
                            ...
                            { // If remove
                                ...
                                {
                                    fprintf(stdout, "FinalLifeFile failed line 944\n");
                                    return 1;
                                }
                                ...
                                if (ffcc != 0)
                                {
                                    fprintf(stdout, "fclose failed 951");
                                    return (1);
                                }
                            ...
                            { // try to move
                                ...
                                {                                                          // if I didn't move
                                    ...
                                    // print to file
                                    sprintf(cohloceachname, "%sCohLocsEach_%s_%d.txt", WorkPath, outputfilenames, loopnum[cohind - 1]);
                                    CohLocEach = fopen(cohloceachname, "a");
                                    if (CohLocEach == NULL)
                                    {
                                        fprintf(stdout, "CohLocEach failed line 983\n");
                                        return 1;
                                    }
                                    fprintf(CohLocEach, "%d\n", newLocs[0]);
                                    fprintf(CohLocEach, "%d\n", newLocs[1]); // last line
                                    ffcc = fclose(CohLocEach);
                                    if (ffcc != 0)
                                    {
                                        fprintf(stdout, "fclose failed 991");
                                        return (1);
                                    }
                                    // write to cohfluc file
                                    CohCOM = getCohCOM(CohCOM, allcoords, newLocs[0], newLocs[1]);
                                    sprintf(cohflucname, "%sCohFluc_%s_%d.txt", WorkPath, outputfilenames, loopnum[cohind - 1]);
                                    CohFluc = fopen(cohflucname, "a");
                                    if (CohFluc == NULL)
                                    {
                                        fprintf(stdout, "CohFluc failed line 1000\n");
                                        return 1;
                                    }
                                    fprintf(CohFluc, "%.2f %.2f %.2f\n", CohCOM[0], CohCOM[1], CohCOM[2]);
                                    ffcc = fclose(CohFluc);
                                    if (ffcc != 0)
                                    {
                                        fprintf(stdout, "fclose failed 1007");
                                        return (1);
                                    }
                                }
                                else if (newLocs[0] == 0)
                                { // if fell off
                                    ...
                                    if (FinalLifeFile == NULL)
                                    {
                                        fprintf(stdout, "FinalLifeFile failed line 1016\n");
                                        return 1;
                                    }
                                    fprintf(FinalLifeFile, "%d %d\n", CohLifetime[cohind - 1], cohlocs[cohind - 1 + maxCoh] - cohlocs[cohind - 1]);
                                    ffcc = fclose(FinalLifeFile);
                                    if (ffcc != 0)
                                    {
                                        fprintf(stdout, "fclose failed 1023");
                                        return (1);
                                    }
                                    CohLifetime[cohind - 1] = 0; // reset coh lifetime
                                    stopflags[cohind - 1] = 0;   // reset stop flags
                                ...
                                {                                                          // if I actually move
                                    ...
                                    if (CohLocEach == NULL)
                                    {
                                        fprintf(stdout, "CohLoceach failed line 1067\n");
                                        return 1;
                                    }
                                    fprintf(CohLocEach, "%d\n", newLocs[0]);
                                    fprintf(CohLocEach, "%d\n", newLocs[1]); // last line
                                    ...
                                    if (ffcc != 0)
                                    {
                                        fprintf(stdout, "fclose failed 1075");
                                        return (1);
                                    }
                                    // write to cohfluc file
                                    CohCOM = getCohCOM(CohCOM, allcoords, newLocs[0], newLocs[1]);
                                    ...
                                    if (CohFluc == NULL)
                                    {
                                        fprintf(stdout, "CohFluc failed line 1084\n");
                                        return 1;
                                    }
                                    fprintf(CohFluc, "%.2f %.2f %.2f\n", CohCOM[0], CohCOM[1], CohCOM[2]);
                                    ffcc = fclose(CohFluc);
                                    if (ffcc != 0)
                                    {
                                        fprintf(stdout, "fclose failed 1091");
                                        return (1);
                                    }
                                }
                        ...
                        { // if something is bound
                            ...
                            { // if try to move
                                ...
                                {                                                          // if I didn't move
                                    ...
                                    if (CohLocEach == NULL)
                                    {
                                        fprintf(stdout, "CohLoceachfailed line 1123\n");
                                        return 1;
                                    }
                                    fprintf(CohLocEach, "%d\n", newLocs[0]);
                                    fprintf(CohLocEach, "%d\n", newLocs[1]); // last line
                                    ...
                                    if (ffcc != 0)
                                    {
                                        fprintf(stdout, "fclose failed 1131");
                                        return (1);
                                    }
                                    // write to cohfluc file
                                    CohCOM = getCohCOM(CohCOM, allcoords, newLocs[0], newLocs[1]);
                                    ...
                                    if (CohFluc == NULL)
                                    {
                                        fprintf(stdout, "CohFluc failed line 1140\n");
                                        return 1;
                                    }
                                    fprintf(CohFluc, "%.2f %.2f %.2f\n", CohCOM[0], CohCOM[1], CohCOM[2]);
                                    ffcc = fclose(CohFluc);
                                    if (ffcc != 0)
                                    {
                                        fprintf(stdout, "fclose failed 1147");
                                        return (1);
                                    }
                                }
                                else if (newLocs[0] == 0)
                                { // if fell off
                                    ...
                                    if (FinalLifeFile == NULL)
                                    {
                                        fprintf(stdout, "FinalLifeFile failed line 1156\n");
                                        return 1;
                                    }
                                    fprintf(FinalLifeFile, "%d %d\n", CohLifetime[cohind - 1], cohlocs[cohind - 1 + maxCoh] - cohlocs[cohind - 1]);
                                    ffcc = fclose(FinalLifeFile);
                                    if (ffcc != 0)
                                    {
                                        fprintf(stdout, "fclose failed 1163");
                                        return (1);
                                    }
                                    CohLifetime[cohind - 1] = 0; // reset coh lifetime
                                    stopflags[cohind - 1] = 0;   // reset stop flags
                                ...
                                {                                                          // if I actually move
                                    ...
                                    if (CohLocEach == NULL)
                                    {
                                        fprintf(stdout, "CohLocEach failed line 1207\n");
                                        return 1;
                                    }
                                    fprintf(CohLocEach, "%d\n", newLocs[0]);
                                    fprintf(CohLocEach, "%d\n", newLocs[1]); // last line
                                    ...
                                    if (ffcc != 0)
                                    {
                                        fprintf(stdout, "fclose failed 1215");
                                        return (1);
                                    }
                                    // write to cohfluc file
                                    CohCOM = getCohCOM(CohCOM, allcoords, newLocs[0], newLocs[1]);
                                    ...
                                    if (CohFluc == NULL)
                                    {
                                        fprintf(stdout, "CohFluc failed line 1224\n");
                                        return 1;
                                    }
                                    fprintf(CohFluc, "%.2f %.2f %.2f\n", CohCOM[0], CohCOM[1], CohCOM[2]);
                                    ffcc = fclose(CohFluc);
                                    if (ffcc != 0)
                                    {
                                        fprintf(stdout, "fclose failed 1231");
                                        return (1);
                                    }
                                }
                                cohlocs[cohind - 1] = newLocs[0]; // update cohlocs array
                            ...
                            { // otherwise, unbind
                                ...
                                if (FinalLifeFile == NULL)
                                {
                                    fprintf(stdout, "FinalLifeFile failed line 1243\n");
                                    return 1;
                                }
                                fprintf(FinalLifeFile, "%d %d\n", CohLifetime[cohind - 1], cohlocs[cohind - 1 + maxCoh] - cohlocs[cohind - 1]);
                                ffcc = fclose(FinalLifeFile);
                                if (ffcc != 0)
                                {
                                    fprintf(stdout, "fclose failed 1250\n");
                                    return (1);
                                }
                                CohLifetime[cohind - 1] = 0; // reset coh lifetime
                                stopflags[cohind - 1] = 0;   // reset stop flags
            ...
            {                     // if I add cohesins
                ...
                { // for each cohindex
                    ...
                    {                                       // if nothing bound at this jj AND I did not just delete it
                        ...
                        // create new cohlocseach file and write coh
                        sprintf(cohloceachname, "%sCohLocsEach_%s_%d.txt", WorkPath, outputfilenames, loopnum[jj]);
                        CohLocEach = fopen(cohloceachname, "a");
                        if (CohLocEach == NULL)
                        {
                            fprintf(stdout, "CohLocEach failed line 1321\n");
                            return 1;
                        }
                        fprintf(CohLocEach, "%d\n", cohlocs[jj]);
                        fprintf(CohLocEach, "%d\n", cohlocs[jj + maxCoh]);
                        ...
                        if (ffcc != 0)
                        {
                            fprintf(stdout, "fclose failed 1329");
                            return (1);
                        }
                        // write to bind file
                        ...
                        if (BindFile == NULL)
                        {
                            fprintf(stdout, "BindFile failed line 1336\n");
                            return 1;
                        }
                        ...
                        if (ffcc != 0)
                        {
                            fprintf(stdout, "fclose failed 1343\n");
                            return (1);
                        }
                        // write to cohfluc file
                        CohCOM = getCohCOM(CohCOM, allcoords, cohlocs[jj], cohlocs[jj + maxCoh]);
                        sprintf(cohflucname, "%sCohFluc_%s_%d.txt", WorkPath, outputfilenames, loopnum[jj]);
                        CohFluc = fopen(cohflucname, "a");
                        if (CohFluc == NULL)
                        {
                            fprintf(stdout, "CohFluc failed line 1352\n");
                            return 1;
                        }
                        fprintf(CohFluc, "%.2f %.2f %.2f\n", CohCOM[0], CohCOM[1], CohCOM[2]);
                        ffcc = fclose(CohFluc);
                        if (ffcc != 0)
                        {
                            fprintf(stdout, "fclose failed 1359");
                            return (1);
                        }
        ...
        // undump and dump for unbinding/binding
        for (int rr = 0; rr < maxCoh; rr++)
        {
            // if something unbinded, undump
            if (yesunbind[rr] == 1)
            {
                sprintf(undumpcommand, "undump %d", loopnum[rr] + 3);
                lammps_command(lmp, undumpcommand);
            }
            // if something bound, change the dump command
            if (yesbind[rr] == 1)
            {
                sprintf(dumpcommand, "dump %d all custom %d %scoordsLEUnwrap_%s_%d.txt id type xu yu zu", loopnum[rr] + 3, dtperdump, WorkPath, outputfilenames, loopnum[rr]);
                lammps_command(lmp, dumpcommand);
                sprintf(dumpmodcommand, "dump_modify %d sort id append yes", loopnum[rr] + 3);
                lammps_command(lmp, dumpmodcommand);
            }
        }
        ...
        // calculate R2, =loadcohdist
        if (myrank == R2rank)
        {
            R2mat = getR2mat(R2mat, allcoords, NumBeads);
        }

        // Update R2 matrix every contactr2freq steps. Rewrite file. Rembember that we are re-averaging to make sure we don't add too much
        if (myrank == R2rank)
        {
            if ((step % contactr2freq) == 0 || step == NumBlocks)
            {
                sprintf(R2name, "%sR2_%s.txt", WorkPath, outputfilenames);
                R2File = fopen(R2name, "w");
                if (R2File == NULL)
                {
                    fprintf(stdout, "R2File failed line 1521\n");
                    return 1;
                }
                for (int uu = 1; uu <= NumBeads; uu++)
                {
                    for (int nn = uu; nn <= NumBeads; nn++)
                    {
                        ...
                        double val = (oldR2mat[thisind] * (step - contactr2freq) + R2mat[thisind]) / (step);
                ...
                if (ffcc != 0)
                {
                    fprintf(stdout, "fclose failed 1538");
                    return (1);
                }
            }
        }

        // Update output contacts every contactr2freq steps. Rewrite file
        if (myrank == contactrank)
        {
            if ((step % contactr2freq) == 0 || step == NumBlocks)
            {
                ...
                if (ContactFile == NULL)
                {
                    fprintf(stdout, "ContactFile failed line 1553\n");
                    return 1;
                }
                for (int uu = 1; uu <= NumBeads; uu++)
                {
                ...
                if (ffcc != 0)
                {
                    fprintf(stdout, "fclose failed 1567");
                    return (1);
                }
            }
        }
        ...
        if (myrank == statusrank)
        {
            if ((step % 10000) == 0)
            {
                ...
                if (statusFile == NULL)
                {
                    fprintf(stdout, "statusFile failed line 1581\n");
                    return 1;
                }
                ...
                if (ffcc != 0)
                {
                    fprintf(stdout, "fclose failed 1588");
                    return (1);
                }
            }
            if (step == round(0.25 * NumBlocks))
            {
                ...
                if (statusFile == NULL)
                {
                    fprintf(stdout, "statusFile failed line 1597\n");
                    return 1;
                }
                ...
                if (ffcc != 0)
                {
                    fprintf(stdout, "fclose failed 1604\n");
                    return (1);
                }
            }
            else if (step == round(0.5 * NumBlocks))
            {
                ...
                if (statusFile == NULL)
                {
                    fprintf(stdout, "statusFile failed line 1613\n");
                    return 1;
                }
                ...
                if (ffcc != 0)
                {
                    fprintf(stdout, "fclose failed 1621");
                    return (1);
                }
            }
            else if (step == round(0.75 * NumBlocks))
            {
                ...
                if (statusFile == NULL)
                {
                    fprintf(stdout, "statusFile failed line 1629\n");
                    return 1;
                }
                ...
                if (ffcc != 0)
                {
                    fprintf(stdout, "fclose failed 1636");
                    return (1);
                }
            }
            else if (step == (NumBlocks - 1))
            {
                ...
                if (statusFile == NULL)
                {
                    fprintf(stdout, "statusFile failed line 1645\n");
                    return 1;
                }
                ...
                if (ffcc != 0)
                {
                    fprintf(stdout, "fclose failed 1652");
                    return (1);
                }
            }
        }

        // print cohlocs, cohlife, numcoh, numatt, numacc from root
        if (myrank == 0)
        {
            ...
            if (NumCohFile == NULL)
            {
                fprintf(stdout, "NumCohFile failed line 1666\n");
                return 1;
            }
            ...
            if (ffcc != 0)
            {
                fprintf(stdout, "fclose failed 1673\n");
                return (1);
            }

            // cohlife
            CohLifeFile = fopen(cohlifename, "a");
            if (CohLifeFile == NULL)
            {
                fprintf(stdout, "CohLifeFile failed line 1681\n");
                return 1;
            }
            for (int yy = 0; yy < maxCoh - 1; yy++)
            { // for each possible coh except last
                fprintf(CohLifeFile, "%d ", CohLifetime[yy]);
            }
            fprintf(CohLifeFile, "%d\n", CohLifetime[maxCoh - 1]); // last line
            ffcc = fclose(CohLifeFile);
            if (ffcc != 0)
            {
                fprintf(stdout, "fclose failed 1692");
                return (1);
            }

            // cohlocs
            CohLocFile = fopen(cohlocname, "a");
            if (CohLocFile == NULL)
            {
                fprintf(stdout, "CohLocFile failed line 1700\n");
                return 1;
            }
            for (int yy = 0; yy < maxCoh - 1; yy++)
            { // for each possible coh except last, print left side
                fprintf(CohLocFile, "%d ", cohlocs[yy]);
            }
            fprintf(CohLocFile, "%d\n", cohlocs[maxCoh - 1]); // last line

            for (int yy = 0; yy < maxCoh - 1; yy++)
            { // for each possible coh except last, print right side
                fprintf(CohLocFile, "%d ", cohlocs[yy + maxCoh]);
            }
            fprintf(CohLocFile, "%d\n", cohlocs[2 * maxCoh - 1]); // last line
            ffcc = fclose(CohLocFile);
            if (ffcc != 0)
            {
                fprintf(stdout, "fclose failed 1717");
                return (1);
            }
            ...
            if (AccFile == NULL)
            {
                fprintf(stdout, "AccFile failed line 1725\n");
                return 1;
            }
            ...
            if (ffcc != 0)
            {
                fprintf(stdout, "fclose failed 1732");
                return (1);
            }
        } // Finished printing
    ...
    { // print percent accepted
        ...
        if (statusFile == NULL)
        {
            fprintf(stdout, "statusFile failed line 1749\n");
            return 1;
        }
        ...
        if (ffcc != 0)
        {
            fprintf(stdout, "fclose failed 1756");
            return (1);
        }
    }
